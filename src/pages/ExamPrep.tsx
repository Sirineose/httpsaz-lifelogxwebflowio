...
import { Calendar, Target, Clock, BookOpen, ChevronRight, Plus, CheckCircle2, Circle, Loader2, Trash2, X } from "lucide-react";
...
const subjects = ["Mathématiques", "Histoire", "Biologie", "Physique", "Français", "Général"];

export default function ExamPrep() {
  const { exams, loading, createExam, updateExam, deleteExam } = useExams();
  const [selectedExam, setSelectedExam] = useState<Exam | null>(null);
  const { sessions, createSession, updateSession } = useStudySessions(selectedExam?.id);
                ...
                >
                  <div className="flex items-start justify-between gap-2 mb-3">
                    ...
                    <span
                      className={cn(
                        "prago-badge",
                        daysLeft <= 3 ? "prago-badge-warning" : "prago-badge-primary"
                      )}
                    >
                      {daysLeft}j
                    </span>
